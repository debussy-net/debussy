"""
The Debussy backend PostgreSQL database
"""
...
BASE_SQL = resource_file("debussy/sql/base.sql")
FLOW_SQL = resource_file("debussy/sql/flows.sql")
NOFLOW_SQL = resource_file("debussy/sql/noflows.sql")
TOPO_SQL = resource_file("debussy/sql/topo.sql")

class DebussyDb():
    """A representation of Debussy's backend PostgreSQL database."""

    def __init__(self, name, user, base, passwd=None, reconnect=False):
        """name: the name of the database to connect to
           user: the username to use to connect
           base: a file containing the SQL implementation for Debussy's base
           passwd: the password to connect to the database
           reconnect: true to connect to an existing database setup, false
           to load a new instance of Debussy's base into the database"""
        self.name = name
        self.user = user
        self.passwd = passwd
        self.base = base
    ...
    @property
    def cursor(self):
        """returns: a psycopg2 cursor from DebussyDb.conn for the PostgreSQL
           database"""
    ...
    def num_connections(self):
        """Returns the number of existing connections to the database.  If
           there are >1 connections, a new Debussy base implementation cannot be
    ...
    def init(self):
        """Initialize the database with the base Debussy SQL implementation.
           Removes any existing Debussy objects from the database"""
        self.clean()
        self.create()
        self.add_extensions()
    ...
    def load_topo(self, provider):
        """Load a topology from the specified network provider
           provider: a debussy.network.NetworkProvider instance"""
        topo = provider.topo
        try:
            node_count = 0
            nodes = {}
    ...
    def add_extensions(self):
        """If not already added, add extensions required by Debussy (plpythonu,
           postgis, pgrouting)"""
        try:
    ...
    def clean(self):
        """Clean the database of any existing Debussy components"""
    ...
    def truncate(self):
        """Clean the database of any state Debussy components, except for
