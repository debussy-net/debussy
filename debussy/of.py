...
class OfManager(object):
    ...
    def registerReceiver(self, receiver):
        """Add a new message receiver
           receiver: a debussy.messaging.MessageReceiver instance"""
    ...
    def sendFlowmod(self, msg):
        """Send a flow modification to the underlying controller implementation
           msg: a debussy.flow.OfMessage instance"""
        pass
...
class PoxInstance(object):
    ...
    def start(self, cargs=None):
        ...
        pox = os.path.join(debussy.util.Config.PoxDir, "pox.py")
        if not os.path.exists(pox):
            logger.error("cannot find pox.py at: {0}. "
                         "Is PoxDir set correctly in debussy.cfg?"
                         .format(pox))
            sys.exit(0)
        ...
        if cargs is None:
            cargs = ["log.level",
                     "--DEBUG",
                     "openflow.of_01",
                     "--port={0}".format(debussy.util.Config.PoxPort),
                     self.app,
                     "openflow.discovery"]

        debussy.util.append_path(debussy.util.resource_file())
        env = os.environ.copy()
