"""
The executing environment for the Debussy CLI
"""
...
import debussy.util
from debussy.app import Application
...
class Environment(object):
    ...
    def start(self):
        ...
        if self.db.cleaned:
            ...
            debussy.util.update_trigger_path(debussy.db.FLOW_SQL,
                                           debussy.util.resource_file())
            self.db.load_schema(debussy.db.FLOW_SQL)

            # delay loading topo triggers until after db is loaded
            # we only want to catch updates after initial load
            debussy.util.update_trigger_path(debussy.db.TOPO_SQL,
                                           debussy.util.resource_file())
            self.db.load_schema(debussy.db.TOPO_SQL)
        else:
            logger.debug("connecting to existing db, skipping load_topo()")

        # if running onlydb mode, remove network flow triggers
        if self.opts.onlydb:
            self.db.load_schema(debussy.db.NOFLOW_SQL)
    ...
    def stop(self):
        ...
        debussy.messaging.clear_queue(Config.QueueId)
