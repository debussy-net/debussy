"""
Debussy application abstractions
"""
...
def mk_watchcmd(db, args):
    """Construct a watch command for a psql query given a list of tables
       db: debussy.db.DebussyDb instance on which to execute the SQL query
...
class AppConsole(cmd.Cmd):
    ...
    def __init__(self, db, env, components):
        """db: a debussy.db.DebussyDb instance
           env: a debussy.env.Environment instance of the CLI's executing environment
...
class AppComponent(object):
    ...
    def drop(self, db):
        """Drop the component from the specified database
           db: a debussy.db.DebussyDb instance containing the component"""
        try:
...
class Application(object):
    """A Debussy application.  In Debussy, an application contains a SQL
       ...
       from the main Debussy CLI by loading the application and typing its name
       or shortcut."""
    ...
    def load(self, db):
        """Load the application from the specified database
           db: a debussy.db.DebussyDb instance into which the application will be loaded"""
    ...
    def unload(self, db):
        """Unload the application from the specified database
           db: a debussy.db.DebussyDb instance containing the application"""
    ...
    def init(self, db, env):
        """Initialize the application without loading it into the database.
           db: a debussy.db.DebussyDb instance to be passed to the application's
           sub-shell
           env: a debussy.env.Environment instance of the CLI's executing
        ...
        debussy.util.append_path(filepath)
