...
through a command line or script).  Flow modifications for the Debussy backend
(database) use the debussy.flow package.
"""
...
class NetworkProvider(object):
    """Superclass for a network provider.  A network provider exposes the
       underlying topology to Debussy's database and CLI.  It also receives
    ...
    def __init__(self, queue_id, db):
        """queue_id: an integer id to be used for the database to communicate
           ...
           database
           db: a debussy.db.DebussyDb instance"""
        self.db = db
        self.receiver = debussy.messaging.MsgQueueReceiver(queue_id, self)
        self.cache_name = {}
        self.cache_id = {}
    ...
    def cli(self, cmd):
        """Pass commands to the network provider's CLI, if it has its own.
           Preferably, the behavior should follow the same pattern as Debussy's
...
class AddLinkMessage(debussy.messaging.ConsumableMessage):
...
class RemoveLinkMessage(debussy.messaging.ConsumableMessage):
...
class AddSwitchMessage(debussy.messaging.ConsumableMessage):
...
class RemoveSwitchMessage(debussy.messaging.ConsumableMessage):
...
class AddHostMessage(debussy.messaging.ConsumableMessage):
...
class RemoveHostMessage(debussy.messaging.ConsumableMessage):
